...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/cli_org/cli_orgSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureCli_orgCols';
...
const TableSampleCli_org = ({
  ...
  const {
    ...
    loading,
    count,
    notify: cli_orgNotify,
    refetch,
  } = useAppSelector((state) => state.cli_org);
  ...
  useEffect(() => {
    if (cli_orgNotify.showNotification) {
      notify(cli_orgNotify.typeNotification, cli_orgNotify.textNotification);
    }
  }, [cli_orgNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/cli_org/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/cli_org/cli_org-view/?id=${id}`);
  };
...
export default TableSampleCli_org;
