...
import { loadColumns } from './configureCli_org_grpCols';
...
const TableSampleCli_org_grp = ({
  ...
  const {
    ...
    loading,
    count,
    notify: cli_org_grpNotify,
    refetch,
  } = useAppSelector((state) => state.cli_org_grp);
  ...
  useEffect(() => {
    if (cli_org_grpNotify.showNotification) {
      notify(
        cli_org_grpNotify.typeNotification,
        cli_org_grpNotify.textNotification,
      );
    }
  }, [cli_org_grpNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/cli_org_grp/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/cli_org_grp/cli_org_grp-view/?id=${id}`);
  };
...
export default TableSampleCli_org_grp;
