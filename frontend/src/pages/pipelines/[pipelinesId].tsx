...
const EditPipelines = () => {
  ...
  const { pipelinesId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: pipelinesId }));
  }, [pipelinesId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: pipelinesId, data }));
...
EditPipelines.getLayout = function getLayout(page: ReactElement) {
  return (
